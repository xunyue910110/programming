	...
	public:
		...
		int getFromDb(const string& key)
		{
		    ...
		    while ( dao.Fetch() )
		    {
		        ...
		        /*strcpy((precord+i)->value1, s1.c_str());
		        strcpy((precord+i)->value2, s2.c_str());
		        strcpy((precord+i)->value3, s3.c_str());
		        strcpy((precord+i)->sresult, s4.c_str()); */
...
IpcParam::IpcParam()
try
:_ipcshm(IpcShm::getShm(CPAR_IPCKEY_)), _ipclck(IpcLock::getLock(CPAR_IPCKEY_))	
{
...
int IpcParam::load(const string& key)
{
	...
	try
	{
	    ...
	    string updatedTime("19700101000000");
		...
		m_parindex.extoff = m_seghead.recsize;		// re-use, for case of refresh & re-load.
		m_parindex.lockid = _ipclck.rqstLock();
		m_parindex.recflag = 1;
		m_parindex.capacity = capacity;
		m_parindex.rinterval = rfInterval; // 5;		//?
...
int IpcParam::refresh(const string& key)
{
	...
	{
		__PAR_TRACE_P1("Refresh %s exit, key refreshing.", key.c_str());
		return -1;
	}
	...
	try
	{
		...
		if ( !isRefreshTime() ) 
		{
			...
			__PAR_TRACE_P1("Refresh %s exit, not to refresh time.", key.c_str());
			return 0;			
		}
...
bool IpcParam::isUpdated()
{
    ...
    if ( dao.Fetch() ) {
	    ...
	    if (strcmp(updatedTime.c_str(), m_parindex.sutime) > 0 )
	    {
	        return true;
	    }
    }    
...
bool IpcParam::isRefreshTime()
{
    ...
    return (elapsedMins < m_parindex.rinterval) ? false : true;
}
...
int IpcParam::__getTrans(const string& key, TParRecord& rec)
{
	...
	///
#ifdef OFF_PAR_REFRESH
	strcpy(rec.sresult, "");
#endif

	/// 如首次未找到则刷新一次，再找，如还未找到则增加扩展空值
	_ipclck.slock(m_parindex.lockid);
	
	int pos = _ParamHelper::binsrch(_hrec, m_parindex.reccnt, rec);
	if (pos < 0) 
	{
		pos = _ParamHelper::trvsrch(_hrecext, m_parindex.extcnt, rec);
	}	
	...
	if (pos < 0)
	{
		if (isRefreshTime() && refresh(key)>0)
		{
...
int IpcParam::__getTrans2(TParRecord& rec)
{
	...
	if (pos < 0) 
	{
		pos = _ParamHelper::trvsrch(m_hext2, m_parindex.extcnt, rec);
	}	
	...
	if (pos < 0)
	{
		if (isRefreshTime() && refresh(m_parindex.skey)>0)
		{
			_ipclck.slock(m_parindex.lockid, 0);
			pos = _ParamHelper::binsrch(m_hrec2, m_parindex.reccnt, rec);
			_ipclck.sunlock(m_parindex.lockid);
		}
...
TParIndex IpcParam::keyinfo(const string& key, int outflag, ostream* osptr)
{
	...
	{	
		...
		*osptr << "======================================================================" << endl;
		...
		sprintf(smsg, "\t lockid: %2d; lrtime: %d; rinterval: %d, capacity: %d",
			  m_parindex.lockid, m_parindex.lrtime, m_parindex.rinterval,m_parindex.capacity);
		*osptr << smsg << endl;
		sprintf(smsg, "\trecflag: %2d; sutime: %s",
			 m_parindex.recflag, m_parindex.sutime);
		*osptr << smsg << endl;
