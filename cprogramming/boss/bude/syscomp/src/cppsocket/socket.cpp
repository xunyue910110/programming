...
#if (defined(hpux) || defined(__hpux__) || defined(__hpux)) && !defined(_XOPEN_SOURCE_EXTENDED)
# undef socklen_t
...
namespace CPPSocket
{
  ...
  void Socket::connect(const sockaddr_in& addr, int timeInMillis) throw (Exception)
  {
  	int error;
    ...
    bool ret = false;
    if(::connect(descriptor, socketAddress, sizeof(addr)) == -1){
      tm.tv_sec = timeInMillis/1000;        
   		tm.tv_usec = (timeInMillis%1000)*1000;
    	FD_ZERO(&set);
      FD_SET(descriptor, &set);
     	if( select(descriptor+1, NULL, &set, NULL, &tm) > 0)
       {
          ::getsockopt(descriptor, SOL_SOCKET, SO_ERROR, &error, (socklen_t *)&len);
          if(error == 0) ret = true;
             else ret = false;
         	} 
         else ret = false;
    ...
    fcntl(descriptor,  F_SETFL, flags);//设置为阻塞模式
   

    if(!ret)//error == CONNECT_ERROR)
    {
      throw LibCException(CONNECT);
    }
  }
  
  void Socket::connect(const Address& addr, int timeInMillis) throw (Exception){
  	connect(static_cast<const sockaddr_in>(addr), timeInMillis);
  }
