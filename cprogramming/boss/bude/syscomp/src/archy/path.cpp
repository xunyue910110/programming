...
// BOOST_POSIX or ARCHY_WINDOWS specify which API to use.
# if !defined( ARCHY_WINDOWS ) && !defined( BOOST_POSIX )
...
#include <string.h>  // SGI MIPSpro compilers need this
#include <vector>
#include <cassert>
    ...
    const path & check_posix_leaf( const path & ph )
    {
      if ( !posix_name( ph.leaf() ) )
        archy::throw_exception( filesystem_error(
          "archy::filesystem::check_posix_leaf",
