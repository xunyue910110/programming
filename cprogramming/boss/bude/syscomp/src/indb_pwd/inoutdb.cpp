...
#else
  	char 		cDbCode[50]; 
  	if(cconfig.getval("DB","DBCODE",cDbCode,50))
  	{
  		printf("DBCODE not found in config file.\n");
  		return false;
  	}
	
  	DBPass dbpass;		
  	DBPassOper* dbPassOper = DBPassOper::getOper();
  	if( ! dbPassOper->GetDBPass(dbpass, cDbCode, username))
  	{
  		printf("config error:GetDBPass error.\n");
  		return false;
  	}  		    
  	strcpy(password,dbpass.DecPasswd.c_str());
  	strcpy(server,dbpass.ServerName.c_str());
  	printf("DB--Source:%s,  Target:%s\n", password,server);
  	delete dbPassOper; 
#endif
  	// over 2007-9-7 9:46
