...
bool IndbApp::getConfig()
{
  ...
  // modified by chenm 2007-9-7 9:46
...
#else
  char 		cDbCode[MAXNAMELEN+1]; 
  if(cconfig.getval("DB","DBCODE",cDbCode,MAXNAMELEN))
  {
    m_theErrorMessages->insert(EAPPFRM_SYSVAL,"config error:DBCODE not define");
    return false;
  }

  DBPass dbpass;		
  DBPassOper* dbPassOper = DBPassOper::getOper();
  if( ! dbPassOper->GetDBPass(dbpass, cDbCode, m_userName))
  {
    m_theErrorMessages->insert(EAPPFRM_SYSVAL,"config error:GetDBPass error");
    return false;
  }  		    
  strcpy(m_passwd,dbpass.DecPasswd.c_str());
  strcpy(m_server,dbpass.ServerName.c_str());
  delete dbPassOper; 
#endif
  // over 2007-9-7 9:46
