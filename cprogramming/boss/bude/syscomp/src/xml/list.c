...
xmlListCreate(xmlListDeallocator deallocator, xmlListDataCompare compare)
{
    ...
    if (deallocator != NULL)
        l->linkDeallocator = (void(*)(xmlLinkPtr))deallocator;
    /* If there is a link comparator, use it */
    if (compare != NULL)
        l->linkCompare = (int(*)(const void *, const void*))compare;
    else /* Use our own */
...
xmlListDup(const xmlListPtr old)
{
     ...
     * the answer
     */
    if (NULL ==(cur = xmlListCreate((xmlListDeallocator)NULL, (xmlListDataCompare)(old->linkCompare))))
        return (NULL);
