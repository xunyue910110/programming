...
{
	return ECParamDAO::getParam(key, value);
}
...
{
	return 	ECParamDAO::getParam(key, value1, value2);
}
...
{
	return ECParamDAO::getParam(key, value1, value2, value3);
}
	...
	if (fldoc < 100) {    
	    if (m_cnt == 0) {
	        for (i=0; i<fldoc; i++) {
	            ...
	            tmpName = ECParamDAO::getParam(key, tmpCode);
	            buf.SetString(toFldName, tmpName, i);
	        }
	    } else if (m_cnt == 1) {
	        for (i=0; i<fldoc; i++) {
	            tmpCode = buf.GetString(fromFldName, i);
	            tmpName = ECParamDAO::getParam(key, value1, tmpCode);
	            buf.SetString(toFldName, tmpName, i);
	        }
	    } else if (m_cnt == 2) {
	        for (i=0; i<fldoc; i++) {
	            tmpCode = buf.GetString(fromFldName, i);
	            tmpName = ECParamDAO::getParam(key, value1, value2, tmpCode);
	            buf.SetString(toFldName, tmpName, i);
	        }
	    }
	} else {
...
int ConvertCodeToName(CFmlBuf& buf, const string& skey, const char* fromFldName, const char* toFldName)
{
	...
	if (fldoc < 100) {
		for (i=0; i<fldoc; i++) {
			tmpCode = buf.GetString(fromFldName, i);
			tmpName = ECParamDAO::getParam(skey, tmpCode);
			buf.SetString(toFldName, tmpName, i);
		}
	} else {
		CFmlBuf abuf1;
...
int ConvertCodeToName(CFmlBuf& buf, const string& skey, const char* fromFldName1, const char* fromFldName2, const char* toFldName)
{
	...
	if (fldoc < 100) {
		for (i=0; i<fldoc; i++) {
			...
			tmpName = ECParamDAO::getParam(skey, tmpCode1, tmpCode2);
			buf.SetString(toFldName, tmpName, i);
		}
	} else {
		CFmlBuf abuf1;
