...
using flowline::Context;
using flowline::RowJSContainer;
using flowline::RowJavaScript;  
/*  
extern "C" {
/////////////////////////////////////////////////////////////////////////////////////////////////////
////  DataSet::Iterator    
...
void RowJavaScript::execute(string& data) {
    if(!good())
        THROW_P2(DefinitionException, "JavaScript不可用:\n, %s, \n%s", getText().c_str(), getErrorInfo().c_str());
...
int main() {
	try {
	DataSet ds, ds2;
	ds.addField(dataset::Field("f1", FTINT,8));
	ds.addField(dataset::Field("f2", FTDOUBLE,10));
	ds.addField(dataset::Field("f3", FTSTRING, 32));
	ds2.addField(dataset::Field("f1", FTINT,8));
	ds2.addField(dataset::Field("f2", FTDOUBLE,10));
	ds2.addField(dataset::Field("f3", FTSTRING, 32));
	...
	MyContext ctx;
	RowJavaScript& rjs = container1.get("ctx.f3+src.f3");//"src.getField('f3')+(src.getField('f2')+src.getField('f1'))+ctx1.getAttribute('aaa')+','");//(&container, ss.c_str());
	container1.setGlobalObject("ctx", RowJavaScript::ctxClass() ,&ctx);
	container1.setGlobalObject("src", RowJavaScript::rowClass() ,&it);
