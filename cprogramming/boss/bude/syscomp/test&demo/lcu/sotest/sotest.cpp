...
PCFmlBuf MWSOTEST::testExecute(PCFmlBuf inBuf){   
    ...
    PCFmlBuf outBuf = PCFmlBuf(new CFmlBuf());//为空会core dump
   	string processname;	 
   	string errinfo;
   	...
   	bool logflag  = false;
   	bool undoflag = false;
   	Logger& tradelog = Logger::getLogger("com.sotest");    
   	try { 
		...
		if(inBuf->IsFldExist(LCU_FML_PROCESS_NAME)){
   	   ...
   	   cout << "[sotest]测试执行开始:" << processname <<  endl; 
   	   	    
   	   LOG_TRACE_P1(tradelog,"[sotest]测试执行开始:%s",processname.c_str()); 
	   if(tradelog.isTraceEnabled()){
		    const string& logtrace = inBuf->ToString();
		    LOG_TRACE_P1(tradelog,"[sotest]测试主流程InBuf:\n%s",logtrace.c_str());
	   }	   
   		//是否执行Undo
        ...
        //       undoflag = true; 
   		...
   		string transAttribute = "true";//"false";
   	...
   	if(errcode != 0){
   	   LOG_TRACE_P1(tradelog,"测试获得异常:\n%s",errinfo.c_str());
   	   cout << errinfo <<endl;
   	   outBuf = inBuf;//如果有异常就取输入buffer返回
    }
    if(errcode == 0 && !outBuf->IsFldExist(LCU_FML_RESULTERRINFO)) {
   		outBuf->SetString(LCU_FML_RESULTERRINFO,"执行成功！");
   		 LOG_TRACE_P1(tradelog,"执行测试成功:\n%s",processname.c_str());
   	}
   	...
   	if(tradelog.isTraceEnabled()){
         ...
         LOG_TRACE_P1(tradelog,"测试主流程OutBuf:\n%s",logtrace.c_str());
    }
...
void SOActionHandle::configure(const string & configuration) {	
      ...
      if (pos == string::npos) throw definition_error(configuration + ", 协议错误：没有找到函数或函数名");
      this->libname = configuration.substr(0, pos);
      int psc = configuration.find("(");
      if (psc == string::npos) {   
      	 this->requestparam = "b";
      	 this->responseparam = "b";
      	 return ;}
