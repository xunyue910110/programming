...
extern "C" void MWSVC(TPSVCINFO *rqst)
{
        ...
        {		
            ...
            tt=time(NULL);
            t=*localtime(&tt);
            sprintf(tmp,"%d-%02d-%02d %02d:%02d:%02d",t.tm_year+1900,t.tm_mon+1,t.tm_mday,t.tm_hour,t.tm_min,t.tm_sec);
            begintime = tmp;
	...
	*/			
		    ...
		    if(!inBuf->IsFldExist("X_LCU_LOGSTEP"))
		        bPrintLog = true;
		    
		    if(bPrintLog)
		    {
		        interfaceLog.setBeginTime(begintime);
		    }
			...
			LOG_TRACE_P1(logMwsvc, "----%s------",begintime.c_str());
			//接触信息生成
	      ...
	      //内存检查代码Begin................
		    ...
		    if(bPrintLog)
		    {
		        ...
		        if(interfaceLog.Init(inBuf,outBuf))
		        {
		            if(interfaceLog.setLogFilePath())
		            {
		                if(interfaceLog.setLogFileName()){
		                		PCFmlBuf logBuf(new CFmlBuf(srcBuf));
		                   	interfaceLog.writeLog(logBuf,outBuf);
		                   	logBuf->SetData(NULL);
		                   }
		            }
                }
