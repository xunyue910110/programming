#ifndef __TD_M_STAFF_UPDATEMOD_LOG_H_
#define __TD_M_STAFF_UPDATEMOD_LOG_H_
...
/** @class StaffUpdatemodLog
 *  @brief TD_M_STAFF_UPDATEMOD_LOG表的实体类
 *  映射数据库TD_M_STAFF_UPDATEMOD_LOG表的所有字段为数据成员，
...
public:
	//! 缺省构造函数，目前没有实现
    StaffUpdatemodLog() { init(); };

	//! 析构函数    
    virtual ~StaffUpdatemodLog() { };
    ...
    //! << 运算符重载
    friend ostream& operator<<(ostream& out, const StaffUpdatemodLog& rhs)
	{
...
/** @brief StaffUpdatemodLog对象指针的vector类型定义
 *  对象数组，用于保存查询结果为多条记录时的数据
 */
typedef vector<StaffUpdatemodLog> VStaffUpdatemodLog;


/**=~===============SEGMENG4==========================*/

/** @class StaffUpdatemodLogDAO
 *  @brief TD_M_STAFF_UPDATEMOD_LOG表的数据访问接口
 *  对TD_M_STAFF_UPDATEMOD_LOG表的数据库访问进行封装，
 *  提供所有SQL执行方法，方便在业务组件中直接引用
 */
class StaffUpdatemodLogDAO : public MTable {
public:
	//! 缺省构造函数
    StaffUpdatemodLogDAO(): MTable("TD_M_STAFF_UPDATEMOD_LOG") { };
    
    //! 析构函数
    virtual ~StaffUpdatemodLogDAO() { };
    
    //! 返回类名
    const char* ClassName() { return "StaffUpdatemodLogDAO"; }
    
    //! 多条记录查询函数，返回实体对象数组
    int jselect(StaffUpdatemodLog &rStaffUpdatemodLog, const string &selBy);

    //! 单条记录查询函数，返回实体对象
    int jselect(VStaffUpdatemodLog &vrStaffUpdatemodLog, const string &selBy);
    
    //! 批量插入记录函数
    int jinsert(VStaffUpdatemodLog &vrStaffUpdatemodLog, const string& insBy);
...
private:
	...
	StaffUpdatemodLogDAO(const StaffUpdatemodLogDAO& rhs);
	StaffUpdatemodLogDAO& operator=(const StaffUpdatemodLogDAO& rhs);
