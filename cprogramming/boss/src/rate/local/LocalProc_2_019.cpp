...
int RateCdrAttribInit::execute()
{
 ...
 if(m_pcdr[ATTRI_BIZ_TYPE] == BIZ_TYPE_GSM)
 {
    ...
    //Add by yangb For sx REQ12 At 2009/12/31 15:18:15 设置长途话单类型
    if (m_pcdr[ATTRI_BIZ_TYPE] == BIZ_TYPE_FIXVOICE)
    {
        //如果长途话单类型为空则置统一的话单类型，IP电话中两个话单类型都有
        if (m_pcdr[ATTRI_LRECORD_TYPE]=="")
            m_pcdr[ATTRI_LRECORD_TYPE]=m_pcdr[ATTRI_RECORD_TYPE];
        m_pcdr[ATTRI_BRECORD_TYPE]=m_pcdr[ATTRI_RECORD_TYPE];
        
        //added by kongqm 2009.01.02 固网到访地和归属地同 因为小区套餐只判断到访地,所以这里必须改掉
...
int SetOriginFeeFlag::execute()
{
  ...
  if(m_pcdr[ATTRI_BIZ_TYPE] == BIZ_TYPE_GSM)
  {
    if((m_pcdr[ATTRI_SOURCE_TYPE] == SOURCE_TYPE_GSM_CRO )||
       ...
       (m_pcdr[ATTRI_SOURCE_TYPE] == SOURCE_TYPE_GSM_CT ))//Add by yangb For sx 村村通
    { //总部下发的国内国际出访话单
     if((m_pcdr[ATTRI_ROAM_TYPE] == ROAM_TYPE_NONE)&&
        (m_pcdr[ATTRI_SOURCE_TYPE] != SOURCE_TYPE_GSM_CT )) //modify by wangzj 村村通
    ...
    else if(m_pcdr[ATTRI_SOURCE_TYPE] == SOURCE_TYPE_GSM_IVPN )
    {  //IVPN用户设置原始费用
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD1,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD3,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD5,1);
    }
    else if(m_pcdr[ATTRI_SOURCE_TYPE] == SOURCE_TYPE_GSM_COMM )
    {  //用户设置原始费用 商务总机
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD1,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD3,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD5,1);
    }
    else if(m_pcdr[ATTRI_SOURCE_TYPE] == SOURCE_TYPE_GSM_RHYW )
    {  //用户设置原始费用 融合业务
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD1,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD3,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD5,1);
  ...
  { //联通SP包月费业务
    ...
    //m_pcdr[ATTRI_FEEUNIT_1] = 1;
    //一个产品只能按M或者T计费
       if (m_pcdr[ATTRI_SP_FEETYPE] =="T")
       {
           m_pRateInfo->m_hasFeeFlag[2]=false;//如果有T,费用域2就不计费
           m_pcdr[ATTRI_DURATION]=m_pcdr[ATTRI_ORGFEE_FIELD1].value();
           DateTime t_beginDateTime;
           DateTime t_endDateTime;
           t_beginDateTime.set(m_pcdr[ATTRI_BEGIN_DATE].c_str(),m_pcdr[ATTRI_BEGIN_TIME].c_str());
           t_endDateTime = t_beginDateTime+m_pcdr[ATTRI_DURATION].value();
       ...
       else if (m_pcdr[ATTRI_SP_FEETYPE] =="M")
       {
           m_pRateInfo->m_hasFeeFlag[3]=false;//如果有M,费用域3就不计费
           m_pcdr[ATTRI_DURATION]=m_pcdr[ATTRI_ORGFEE_FIELD1].value();
           DateTime t_beginDateTime;
  ...
  { //固网语音类
    ...
    if(m_pcdr[ATTRI_SOURCE_TYPE] == "77" )//400,800业务有原始费用
    {  //用户设置原始费用
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD1,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD3,1);
    	m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD5,1);
  ...
  { //固网短信类
    //Add by yangb For sx At 2009/12/2 23:09:47
    //山西联通固网短信代收费，需要置原始费用
    if( m_pcdr[ATTRI_SOURCE_TYPE] == "2A" && m_pcdr[ATTRI_CALL_TYPE] == CALL_TYPE_SMT)//固网短信代收费,有原始费用
    {
      m_pRateInfo->setOrgFeeFlag(ATTRI_DISCFEE_FIELD2,1);
      m_pcdr[ATTRI_FEEUNIT_2] = 1; 
    }
