#include "PO_BaseTp_Group.h"

bool PO_BaseTp_Group::getProperty(PO_Property &property)
{
	property=m_property;
	return true;
}

bool PO_BaseTp_Group::convert(const StringVector &argname)
{
	int i=0;
	string tmpstr;
...
PO_BaseTp_Group::PO_BaseTp_Group(const PO_BaseTp_Group& right)
{
	*this=right;
}

void PO_BaseTp_Group::operator=(const PO_BaseTp_Group& right)
{
	m_record=right.m_record;
}

int operator==(const PO_BaseTp_Group &left,const PO_BaseTp_Group& right)
{
...
ostream& operator<<(ostream& os,const PO_BaseTp_Group& po)
{
...
PO_Property PO_BaseTp_Group::m_property={FROM_DATABASE,BY_FLAG,
		"","PO_BaseTp_Group",USERNAME_1,PASSWORD_1,SERVNAME_1,
		"select FEEPOLICY_ID,BEGIN_TIME,END_TIME,BIZ_TYPE,HOME_AREA_CODE,\
		 CALL_TYPE,ROAM_TYPE,SERVICE_TYPE,CARRIER_TYPE,rule_id,EFFECT_BIZ_TAG from TD_BASE_TARIFF",
