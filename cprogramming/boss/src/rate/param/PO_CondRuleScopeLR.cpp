...
bool PO_CondRuleScopeLR::getProperty(PO_Property &property)
{
	property = m_property;
	return true;
}
...
bool PO_CondRuleScopeLR::convert(const StringVector &argname)
{
	int i = 0;
	string tmpstr;
...
PO_CondRuleScopeLR::PO_CondRuleScopeLR(const PO_CondRuleScopeLR& right)
{
	*this = right;
}


void PO_CondRuleScopeLR::operator = (const PO_CondRuleScopeLR& right)
{
	m_record = right.m_record;
}


int operator == (const PO_CondRuleScopeLR &left,const PO_CondRuleScopeLR& right)
{
...
ostream& operator << (ostream& os,const PO_CondRuleScopeLR& po)
{
	//os<< po.m_record<<endl; 
...
PO_Property PO_CondRuleScopeLR::m_property = {FROM_DATABASE,BY_FLAG,
    "","PO_CondRuleScopeLR","","","",
    "SELECT COND_ID,YES_OR_NO,JUDGE_OBJECT_ID,DIVIED_TAG,JUDGE_METHOD,"
    "VALUE_METHOD,VALUE_TYPE,VALUE_PARAM,MIN_VALUE,MAX_VALUE "
    "FROM TD_B_SIMPLECOND WHERE JUDGE_METHOD = '3'",
    10,"SELECT count(*) FROM TD_B_SIMPLECOND WHERE JUDGE_METHOD = '3'"};
