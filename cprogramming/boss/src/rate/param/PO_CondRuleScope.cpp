...
bool PO_CondRuleScope::getProperty(PO_Property &property)
{
	property = m_property;
	return true;
}
...
bool PO_CondRuleScope::convert(const StringVector &argname)
{
	int i = 0;
	string tmpstr;
...
PO_CondRuleScope::PO_CondRuleScope(const PO_CondRuleScope& right)
{
	*this = right;
}


void PO_CondRuleScope::operator = (const PO_CondRuleScope& right)
{
	m_record = right.m_record;
}


int operator == (const PO_CondRuleScope &left,const PO_CondRuleScope& right)
{
...
ostream& operator << (ostream& os,const PO_CondRuleScope& po)
{
	//os<< po.m_record<<endl; 
...
PO_Property PO_CondRuleScope::m_property = {FROM_DATABASE,BY_FLAG,
    "","PO_CondRuleScope","","","",
    "SELECT COND_ID,YES_OR_NO,JUDGE_OBJECT_ID,DIVIED_TAG,JUDGE_METHOD,"
    "VALUE_METHOD,VALUE_TYPE,VALUE_PARAM,MIN_VALUE,MAX_VALUE "
    "FROM TD_B_SIMPLECOND WHERE JUDGE_METHOD = '0'",
    10,"SELECT count(*) FROM TD_B_SIMPLECOND WHERE JUDGE_METHOD = '0'"};
