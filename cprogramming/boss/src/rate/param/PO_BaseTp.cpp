...
bool PO_BaseTp::getProperty(PO_Property &property)
{
	property=m_property;
	return true;
}

bool PO_BaseTp::convert(const StringVector &argname)
{
	int i=0;
	string tmpstr;
...
PO_BaseTp::PO_BaseTp(const PO_BaseTp& right)
{
	*this=right;
}

void PO_BaseTp::operator=(const PO_BaseTp& right)
{
	m_record=right.m_record;
}

int operator==(const PO_BaseTp &left,const PO_BaseTp& right)
{
...
ostream& operator<<(ostream& os,const PO_BaseTp& po)
{
...
PO_Property PO_BaseTp::m_property={FROM_DATABASE,BY_FLAG,
		"","PO_BaseTp",USERNAME_1,PASSWORD_1,SERVNAME_1,
		...
		11,"select count(*) from TD_BASE_TARIFF"};
