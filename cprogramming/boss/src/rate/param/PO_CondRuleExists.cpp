...
bool PO_CondRuleExists::getProperty(PO_Property &property)
{
	property = m_property;
	return true;
}
...
bool PO_CondRuleExists::convert(const StringVector &argname)
{
	int i = 0;
	string tmpstr;
...
PO_CondRuleExists::PO_CondRuleExists(const PO_CondRuleExists& right)
{
	*this = right;
}


void PO_CondRuleExists::operator = (const PO_CondRuleExists& right)
{
	m_record = right.m_record;
}


int operator == (const PO_CondRuleExists &left,const PO_CondRuleExists& right)
{
...
ostream& operator << (ostream& os,const PO_CondRuleExists& po)
{
	//os<< po.m_record<<endl; 
...
PO_Property PO_CondRuleExists::m_property = {FROM_DATABASE,BY_FLAG,
    "","PO_CondRuleExists","","","",
    "SELECT COND_ID,YES_OR_NO,JUDGE_OBJECT_ID,DIVIED_TAG,JUDGE_METHOD,"
    "VALUE_METHOD,VALUE_TYPE,VALUE_PARAM,MIN_VALUE,MAX_VALUE,ENUM_VALUE "
    "FROM TD_B_SIMPLECOND WHERE JUDGE_METHOD = '5'",
    11,"SELECT count(*) FROM TD_B_SIMPLECOND WHERE JUDGE_METHOD = '5'"};
