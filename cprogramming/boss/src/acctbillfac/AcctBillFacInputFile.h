...
* 完成日期：
*/
#ifndef AcctBillFacInputFile_H
#define AcctBillFacInputFile_H

#include "AcctBillFacCommon.h"

/**
 * AcctBillFacInputFile：帐户账单输入文件管理.
 * 含有下面的方法:
 *   init:              根据系统配置参数信息初始化输入文件的目录和文件名称
 *   getAcctBill:       从输入文件中读取帐户及帐户帐单信息
 *   openFile:          打开输入文件
 *   closeFile:         关闭输入文件
 *   getDealDate:       从系统获得文件处理时间
 */
class AcctBillFacInputFile
{
	public:
	
	    AcctBillFacInputFile();
         ...
         * @param  vecAcctBill 客户用户帐单.
         * @return 获得到返回true,未获得到返回false.
         */
	    bool getAcctBill(AcctKey& acctKey, vector<AcctBill> &vecAcctBill);
	...
	private:
	    ...
	    T_SERVER_ID             m_lastStrUserId;
	    T_SERVER_ID             m_lastStrAcctId;   
	    //added for js.unicom   
	    String<4>               m_lastEparchyCode;      
	    //end of add
	    long                    m_lastItemCode;
	    long                    m_lastInitFee;
	    long                    m_lastFee;
	    long                    m_lastAdiscount;
	    long                    m_lastadjustBefore;
	    long                    m_lastacctDay;
};

#endif //AcctBillFacInputFile_H
