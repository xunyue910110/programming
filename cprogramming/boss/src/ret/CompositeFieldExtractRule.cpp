...
bool CompositeFieldExtractRule::initialize(vector<string> *argname, FieldExtractRule *aParentsField, const int aFloor)
{
	...
	while (argname->size()>0)
	{
		...
		// 打印调试信息
		m_RetLog_->ShowMessage(sectionName+", new a Field object,",0);
		sprintf(Tmp,"%d",FieldExtRuleTmp->m_theLayer_);
		m_RetLog_->ShowMessage( " the layer is "+string(Tmp),0 );
		m_RetLog_->ShowMessage( ". Parent is "+aParantsName+", the parent layer is ",0 );
		sprintf(Tmp,"%d",aParentsField->m_theLayer_);
		m_RetLog_->ShowMessage( string(Tmp)+"\n",0 );
...
FieldExtractRule *CompositeFieldExtractRule::createChildFieldObject(FieldExtractRule *& aChildField,string aFieldType)
{
	...
	{ 
		aChildField = new FixedFieldExtractRule;
	} 
	...
	//{
		//aChildField = new AsnFieldExtractRule;
	//} 
	//else if (aFieldType.compare("GASN")==0)    	    //创建GASN变长处理字段对象 add by chenm 2005-3-9
	//{
		//aChildField = new GasnFieldExtractRule;
	//} 
	else 
	{
		aChildField = 0;
