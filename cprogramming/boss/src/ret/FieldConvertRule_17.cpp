...
bool FieldConvertRule_17::getFields(const string &ruleStr,string &totalStr,string &srcStr)
{
...
bool FieldConvertRule_17::convertField(multimap<MyString,FieldInfo *> &theFieldInfoMaps)
{
	int len,i;
	     ...
	     &&( (m_sTimes.size()-1)== m_dataFlowDns.size())  )
	{
		...
		{
			if((m_sTimes[0].compare(6,6,"230000") >= 0 && m_sTimes[0].compare(6,6,"240000") <= 0)
			  || (m_sTimes[0].compare(6,6,"000000") >= 0 && m_sTimes[0].compare(6,6,"065959") <= 0))
			{
...
long FieldConvertRule_17::diffSecond(const string & dt1,const string & dt2)
{
    struct tm t;
    time_t t1,t2;
    char tmp[10];
