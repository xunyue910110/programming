...
namespace LINKAGE_PP
{
class ExceptionMsgPp
{
public:
	//??异常发生的文件
	char m_filename[255];
	...
	//??1 成功
	//??0 失败
	ExceptionMsgPp(char * filename, int linenum , char * format, ...);
	
	ExceptionMsgPp(int linenum, char * filename, char * msg)
	{
...
public:
	...
	//??返回值：
	//??无
	Pp_Exception(char * filename, int linenum , const char * format, ...);
	...
	//??返回值：
	//??无
	void Push(ExceptionMsgPp msg);
	
	//??异常信息vector类型
	vector<ExceptionMsgPp> m_msg;
	
	//??异常信息
	char m_errstr[4*1024];

	//??析构函数
	virtual ~Pp_Exception() throw(){}//modify by chenzm for hp-ux


};
}
using namespace LINKAGE_PP;
