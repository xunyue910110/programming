...
int OutputFile::setFileBuffer(string aSrcFileName,unsigned char *&srcFileBuffer)
{
	...
	if(m_srcfd!=0) 
	{
	    if(close(m_srcfd)!=0) 
	    {
	        theErrorMessages->insert(PP_CLOSE_FILE_ERROR,
	                                "致命错误：开始时关闭原始文件"+m_srcFileName+"失败，程序退出 !");
#ifndef PPTEST
	...
	#ifdef DEBUG
            cout<<"m_fileLength is "<<m_fileLength<<endl;
    #endif
	...
	if(m_fileLength==0)                                 //话单文件长度为0。
	{
       ...
       {
	        theErrorMessages->insert(PP_CLOSE_FILE_ERROR,
	                                "致命错误：关闭原始文件(长度0)"+m_srcFileName+"失败，程序退出 !");
#ifndef PPTEST
            g_application.emergyExitAlert();
#endif 
            exit(1);
...
int OutputFile::writeFileHead(int fileno)
{
    return m_StandardFile.writeFileHead(fileno);
}

int OutputFile::writeFileTail(int fileno)
{
    return m_StandardFile.writeFileTail(fileno);
}
...
int OutputFile::write(multimap<MyString,FieldInfo *> &theFieldInfoMaps)
{
    ...
    {                 
       ...
       {                    
          if(m_SendLog_->getBeginCdrNo()==0) 
          {
                m_SendLog_->setBeginCdrNo(cdrNo);
          }
          m_SendLog_->setEndCdrNo(cdrNo);
...
void OutputFile::closeSrcFiles(int flag)
{
    ...
    if( m_SendLog_->getTotalCdrs() > 0 ) 
    {
	    theErrorRate=(float) m_SendLog_->getErrorSum() / (float)m_SendLog_->getTotalCdrs();
...
{
        m_StandardFile.close(flag);
}

//##ModelId=3BB19885038B
// modify by dingj : add parameter outFileHType
bool OutputFile::initialize(vector<string> aStandardFile,vector<string> aStandardFileHead,
	            vector<string> aStandardFileTail,vector<string> aFilePath,
							...
							string fileFreq,string outFileHSize,float errorRate)
{
  ...
  m_errorRate       = errorRate;
	...
	m_StandardFile.initialize(aStandardFile,aStandardFileHead,aStandardFileTail,
	                          isBackupDes,isIndb,outFileType,outFileHType,
	                          fileFreq,outFileHSize,//add for vpdn 2010-4-9 14:28:10 by zhaoly
	                          getPath(aFilePath,"destination_path"),
							              getPath(aFilePath,"des_backup_path"),
							              getPath(aFilePath,"file_error_path"),
							              getPath(aFilePath,"file_indb_path") );
...
void OutputFile::mvToErrDir(string & filename)
{
    ...
    //modified by chenm 2005-1-24 错误文件不输出到错误目录下,直接备份到备份目录下
    //srcErrorName= m_fileErrorPath  + "E" + filename;
...
// 定长格式话单块大小出错文件移至错误目录
void OutputFile::mvToErrorDir(string & filename)
{
    string srcFileName,srcErrorName;
