...
FieldConvertRule_16::~FieldConvertRule_16()
{
    if( m_fDes != 0 )
	{ 
	   string tmpFileName = m_tmpPath + m_PREFIX + m_SendLog->getSrcFileName();
	   //关闭输出文件 
	   if( fclose(m_fDes)!=0 )
	   {
	   ...
	   {
	        string desFileName = m_desPath + m_PREFIX + m_SendLog->getSrcFileName();
...
bool FieldConvertRule_16::convertField(multimap<MyString,FieldInfo *> &theFieldInfoMaps)
{
	...
	//正在处理的文件
	m_thisFileName = m_PREFIX + m_SendLog->getSrcFileName();
