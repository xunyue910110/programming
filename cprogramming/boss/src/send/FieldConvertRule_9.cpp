...
bool FieldConvertRule_9::convertField(multimap<MyString,FieldInfo *> &theFieldInfoMaps)
{
    int len,src1,src2,des;
...
//		pFieldInfo->m_IsCheck 		= true;
		theFieldInfoMaps.insert( multimap<MyString,FieldInfo *>::value_type(m_desFieldName.c_str(),pFieldInfo) );  
    ...
    {
        src1    =   atoi( m_operatLeftFieldName.c_str() );     
    } 
    else 
    {
        src1    =   atoi( srcFieldInfo->m_FieldDesValue.c_str() );   
    }
    
	if( (srcFieldInfo=getFieldMap(theFieldInfoMaps,m_operatRightFieldName))==0 ) 
	{
        src2    =   atoi( m_operatRightFieldName.c_str() );     
    } 
    else 
    {
        src2    =   atoi( srcFieldInfo->m_FieldDesValue.c_str() );   
    }   
