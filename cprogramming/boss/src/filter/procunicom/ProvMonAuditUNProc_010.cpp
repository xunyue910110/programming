#include "ProvMonAuditUNProc_010.h"

#include "FilterConfig.h"
#include "PMGlobalManager.h"
#include "base/TimeUtil.h"

INSTALL_PROCESSBASE_INSTANCE(ProvMonAuditUNProc_010);

bool ProvMonAuditUNProc_010::auditFee()
{
	//本地化稽核处理
...
bool ProvMonAuditUNProc_010::preauditStatus()
{
	int t_ret;
	char    finish_time[20];
	char    start_time[20];
...
bool ProvMonAuditUNProc_010::auditSmStatus()
{
	//包月话单稽核流程
...
bool ProvMonAuditUNProc_010::auditSpStatus()
{
	if(m_stategroup == USER_UNDEFINED) //用户状态找不到
	{
		m_pcdr[ATTRI_ERROR_CODE]=ERROR_NOSTATUS;
...
bool ProvMonAuditUNProc_010::auditppsSpStatus()
{
	if(m_stategroup == USER_UNDEFINED) //用户状态找不到
	{
		m_pcdr[ATTRI_ERROR_CODE]=ERROR_NOSTATUS;
...
bool ProvMonAuditUNProc_010::auditsp()
{
	return true;
}

bool ProvMonAuditUNProc_010::smsaudit()
{
...
bool ProvMonAuditUNProc_010::spaudit()
{
	if(!preauditStatus())
		return false;
...
bool ProvMonAuditUNProc_010::ppsspaudit()
{
	if(!preauditStatus())
		return false;
...
bool ProvMonAuditUNProc_010::monthfeeaudit()
{
	if(!preauditStatus())
		return false;
...
int ProvMonAuditUNProc_010::execute()
{
