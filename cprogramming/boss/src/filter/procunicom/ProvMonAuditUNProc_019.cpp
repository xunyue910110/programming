#include "ProvMonAuditUNProc_019.h"

#include "FilterConfig.h"
#include "PMGlobalManager.h"
#include "base/TimeUtil.h"

INSTALL_PROCESSBASE_INSTANCE(ProvMonAuditUNProc_019);

bool ProvMonAuditUNProc_019::auditFee()
{
  //本地化稽核处理
	if (ProcessBaseHelper::ExecuteProcedure("LocalizeSpOperFeeProc")==1)
		return true;
...
bool ProvMonAuditUNProc_019::preauditStatus()
{
  int t_ret;
  char    finish_time[20];
  char    start_time[20];
  ...
  if (m_pcdr[ATTRI_BIZ_TYPE] == BIZ_TYPE_UNICOMSP)
  	...
  	t_method = m_Config->m_statusAuditMethod;
...
bool ProvMonAuditUNProc_019::auditSmStatus()
{
  //包月话单稽核流程
...
bool ProvMonAuditUNProc_019::auditSpStatus()
{
  if(m_stategroup == USER_UNDEFINED) //用户状态找不到
  {
    m_pcdr[ATTRI_ERROR_CODE]=ERROR_NOSTATUS;
...
bool ProvMonAuditUNProc_019::auditppsSpStatus()
{
  if(m_stategroup == USER_UNDEFINED) //用户状态找不到
  {
    m_pcdr[ATTRI_ERROR_CODE]=ERROR_NOSTATUS;
...
bool ProvMonAuditUNProc_019::auditsp()
{
  return true;
}

bool ProvMonAuditUNProc_019::smsaudit()
{
  //isVerifyState = 'N'; //默认稽核用户状态   by songyx 20090517 将Y-->N
  isVerifyState = 'Y'; //需要稽核用户状态   by songyx 20090904 将N-->Y
  isVerifyOrder = 'N'; //默认稽核订购关系   by songyx 20090517 将Y-->N
...
bool ProvMonAuditUNProc_019::spaudit()
{
  if(!preauditStatus())
    return false;
...
bool ProvMonAuditUNProc_019::ppsspaudit()
{
  if(!preauditStatus())
    return false;
...
bool ProvMonAuditUNProc_019::monthfeeaudit()
{
  if(!preauditStatus())
    return false;
...
int ProvMonAuditUNProc_019::execute()
{
