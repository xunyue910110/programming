#include "LocalizeScpAttribProc_019.h"
...
//	    m_cdr[ATTRI_VISIT_CODE_A] = m_cdr[ATTRI_VISIT_CODE_A].append(0,"0");
	...
	//backup ATTRI_VISIT_CODE_A
		
	//漫游出访话单，根据roam_type区分 --mod by yangyi 2006,12,24,02:03
	...
	//国内漫游出访话单，不求边界漫游类型   modif by zhangl 2009,05,28
	if ( m_cdr[ATTRI_ROAM_TYPE] != ROAM_TYPE_NATIONAL_OUT )
	{
	   edgeRoamProcess();
  }
  ...
  else
  {
	...
	//国内漫游出访话单，不求漫游类型   modif by zhangl 2009,05,28
	if ( m_cdr[ATTRI_ROAM_TYPE] != ROAM_TYPE_NATIONAL_OUT )
	{
		setScpRoamType();
	}
...
{
	if (m_cdr[ATTRI_ROAM_TYPE] == ROAM_TYPE_NONE)
	{
